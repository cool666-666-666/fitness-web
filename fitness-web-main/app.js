(() => {
  ...
  function renderAll() {
    ...
    const latestDayKey = sorted.length
      ? getDateKey(sorted[0].createdAt || sorted[0].created_at)
      : "";
    const latestDayRecords = latestDayKey
      ? sorted.filter(item => getDateKey(item.createdAt || item.created_at) === latestDayKey)
      : [];
    ...
    if (el.emptyTip) {
      el.emptyTip.classList.toggle("hidden", latestDayRecords.length > 0);
    }
